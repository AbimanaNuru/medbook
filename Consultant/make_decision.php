...
if(isset($_POST['save_patient_decision']))
{

$cons_id=$_GET['consultation'];
$consultant_id=$_SESSION['user_id'];
$decision=mysqli_real_escape_string($connection,$_POST['decision_details']);

// Query for insertion data into database  
$query=mysqli_query($connection,"UPDATE consultation SET decision='$decision' WHERE cons_id ='$cons_id' AND consultation_user_id='$consultant_id'");
if($query)
{
	echo " <script> alert('Patient Consultation Decision Successfully') </script> ";
header("Refresh: 0; url= p_laboratory_test.php");
...
<body>
	...
	<div class="main-container">
        ...
        <div class="clearfix">
						<div class="pull-left">
							<h4 class="text-green h4">PATIENT FINAL DECISION</h4></div>				
					</div>
                    <br>
                    <?php
$p_id=$_GET['patient'];
$cons_id=$_GET['consultation'];
$consultant_id=$_SESSION['user_id'];
$query = "SELECT * from patients,laboratory_test,consultation  WHERE laboratory_test.patient_id='$p_id' AND
consultation.cons_id ='$cons_id'  AND laboratory_test.patient_id=patients.p_id AND consultation.cons_id=laboratory_test.consultation_id  ";     
...
<h6> Consultation Details:[<?php echo $patient['date_and_time']; ?>]</h6><br>
<?php echo $patient['patient_consultation_details']; ?></p> 

<h6> Laboratory Text:[<?php echo $patient['test_date_time']; ?>]</h6><br>
<?php echo $patient['laboratory_test_details']; ?></p> 
...
<form action="" method="POST">
					...
					<p>Full Final Decision Details: *</p>
					<textarea class="textarea_editor form-control border-radius-0" name="decision_details" required placeholder="Provide Final Decision Details  ..."></textarea>
				</div>
                
	
	<button class="btn btn-success" type="submit" name="save_patient_decision">SAVE DECISION</button>
</form>
